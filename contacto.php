<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $mail_to = "yisac97@gmail.com";
        
        # Envío de datos
        $subject = "Informacion de Xt - Net";
        $name1 = str_replace(array("\r","\n"),array(" "," ") , strip_tags(trim($_POST["name"])));
        $name2 = stripcslashes($name1);
        $name3 = htmlspecialchars($name1);
        $email = filter_var(trim($_POST["email"]), FILTER_SANITIZE_EMAIL);
        $message1 = trim($_POST["message"]);
        $message2 = stripcslashes($message1);
        $message3 = htmlspecialchars($message2);
        
        if ( empty($name1) OR !filter_var($email, FILTER_VALIDATE_EMAIL) OR empty($message1)) {
        ...
        $content = "Nombre: $name3\n";
        $content .= "E-mail: $email\n\n";
        $content .= "Mensaje:\n$message3\n";

        # Encabezados de correo electrónico.
        $headers = "From: $name3 <$email>";
